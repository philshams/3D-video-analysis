...
session_video_folder = "C:\\Drive\\Video Analysis\\data\\3D_pipeline\\bj\\"
session_video = session_video_folder + 'bj148b_test2.avi'
twoD = True
...
start_frame = 2000; stop_frame = 40000  #13800, session 1...
frame_rate = 1000
...
data_for_model_normalized = np.load(file_location_data_cur+ '_data_for_' + model_type + '_normalized' + twoD_suffix + '.npy')
chosen_components = np.load(file_location_data_cur+ '_chosen_components_' + model_type_and_name_tag + twoD_suffix + '.npy')
components_binary = np.load(file_location_data_cur+ '_components_binary_' + model_type_and_name_tag + twoD_suffix + '.npy')
unchosen_components_binary = np.load(file_location_data_cur+ '_unchosen_components_binary_' + model_type_and_name_tag + twoD_suffix + '.npy')
probabilities = np.load(file_location_data_cur+ '_probabilities_' + model_type_and_name_tag + twoD_suffix + '.npy')
unchosen_probabilities = np.load(file_location_data_cur+ '_unchosen_probabilities_' + model_type_and_name_tag + twoD_suffix + '.npy')

num_clusters = probabilities.shape[1]
model = joblib.load(file_location_data_library + '_' + model_type + '_' + model_type_and_name_tag + twoD_suffix )  # load model

add_velocity, speed_only, add_change, add_turn, num_PCs_used, window_size, windows_to_look_at, feature_max = np.load(
    file_location_data_library + '_' + model_type + '_settings_' + model_type_and_name_tag + twoD_suffix  + '.npy')
...
start_frame_data = int(np.load(save_folder_location + session_name_tag + '\\' + session_name_tag + '_start_end_frame.npy') [0])
together = position_orientation_velocity[:, 1].astype(bool)
frames = position_orientation_velocity[:, 0]
frames = frames[together]

# show a picture of a apart when the mouse enters the apart
try:
    apart = (cv2.imread('C:\\Drive\\Video Analysis\\code\\superorganism-analysis\\apart.jpg') / 3).astype(np.uint8)
    apart = cv2.resize(apart, (450, 450))
except:
    apart = np.zeros((450,450)).astype(np.uint8)
    print('apart image not found')
...
for n in range(num_clusters):
    ...
    for t in range(2 * windows_to_look_at + 1):
        ...
        reconstruction_from_wavelet[(reconstruction_from_wavelet<=0) + twoD*(reconstruction_from_wavelet>=250)] = 0 + twoD*250
        reconstruction_image = cv2.resize(reconstruction_from_wavelet.astype(np.uint8),(trajectory_pose_size, trajectory_pose_size))
...
start_frame_visualization = start_frame + start_frame_data 
mouse_vid.set(cv2.CAP_PROP_POS_FRAMES, start_frame_visualization)
wavelet_array = np.load(save_folder_location + session_name_tag + '\\' + session_name_tag + '_wavelet' + twoD_suffix + '.npy')
...
j = min(np.where(frames >= (start_frame_visualization - window_size * windows_to_look_at))[0])
while True:
    ...
    if ret:
        ...
        if together[i]:  # mice together
            ...
            chosen_color = [0, 0, 0]
            reconstruction_from_wavelet = apart
            if only_see:
                i += 1 
                continue
        i += 1 
        ...
        cv2.putText(mouse_frame, str(i), (50, 450), 0, 1, 255)
