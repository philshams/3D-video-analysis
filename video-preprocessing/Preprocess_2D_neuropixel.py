...
# ------------------------------------------
video_file_location = "C:\\Drive\\Video Analysis\\data\\neuropixel\\08.12.2018\\"
video_name = "Chronic_behav_software-default-2393555-"
videos = [0]

save_folder_location = "C:\\Drive\\Video Analysis\\data\\neuropixel\\"
session_name_tag = "neuropixel_1"  # name-tag to be associated with all saved files
initial_run = True
...
if not use_saved_settings:
    ...
    # ----------------------------
    mask_thresh = .35  # mouse mask threshold (lower is more stringently darker than background)
    kernel = [3, 8]  # erosion and dilation kernel sizes for mouse mask
    iters = [0, 12]  # number of erosion and dilation iterations for mouse mask
...
#          ******** arena-specific section ends here *********
            ...
            if use_norm_to_analyze:
                ...
                frame_masked[frame_masked > 255] = 255
            else:
                if undistort:
            ...
            try:
                if np.sum(frame_norm_mask) < 1650:
                    width_thresh_cur = np.inf
                else:
                    width_thresh_cur = width_thresh
                rotate_angle, current_orientation, ellipse, topright_or_botleft, ellipse_width = \
                    flip_mouse(current_orientation, prev_ellipse, topright_or_botleft, frame_masked_cropped, width_thresh=width_thresh_cur)
            ...
            rotate_angle, current_orientation, depth_ratio, history_x, history_y, head_dir_x_component, head_dir_y_component, flip = \
                correct_flip(frame_num - start_frame, current_orientation,[1], [1], history_x, history_y,
                             x_center, y_center, ellipse, ellipse_width* (np.sum(frame_norm_mask) / 3000), \
