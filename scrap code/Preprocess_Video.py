...
#file_name = 'Depth_Camera-default-491687-video-0.avi'
#file_name_2 = 'Depth_Camera-default-491687-2video-0.avi'
#
#file_loc = 'C:\Drive\Video Analysis\data\\'
#date = '15.03.2018\\'
#mouse_session = 'bj141p2\\'
#save_vid_name = 'test' # name-tag to be associated with all saved files
two_videos = False
video_type = '3D'
...
if two_videos:
    file_name_2 = file_loc + file_name_2
print(file_name + ' loaded')
vid = cv2.VideoCapture(file_name)
if two_videos:
    vid2 = cv2.VideoCapture(file_name_2)
...
kernel = [2,6] #erosion and dilation kernel sizes for mouse mask
iters = [2,2] #number of erosion and dilation iterations for mouse mask
...
stereo_axis = 'x' #axis of the two cameras with respect to the video
depth_being_opposite_not_sure_why = True
...
if do_get_offsets:
    ...
    offset_file_name = file_loc + 'y_offset.npy'
...
if stereo_axis == 'x':
    background_R = cv2.copyMakeBorder(background_mat[:,:,1],top=offset*(offset>0),bottom=offset*(offset<0),left=0,right=0,borderType= cv2.BORDER_REPLICATE,value=0)
    background_R = background_R[offset*(offset<0):background_R.shape[0]-offset*(offset>0),:] 
elif stereo_axis == 'y':
    background_R = cv2.copyMakeBorder(background_mat[:,:,1],top=0,bottom=0,left=offset*(offset>0),right=offset*(offset<0),borderType= cv2.BORDER_REPLICATE,value=0)
    background_R = background_R[:,offset*(offset<0):background_R.shape[1]-offset*(offset>0)]       
...
while True:
    ...
    if ret and ret2:  
        ...
        if two_videos:
            ...
            frame_R = np.zeros(frame_R.shape) #just use one camera
        else:
            frame_R = frame[:,:,r] #stereoscopic camera
        
        # shift the right frame by the y-offset, whose value was calibrated above       
        if stereo_axis == 'x':
            frame_R = cv2.copyMakeBorder(frame_R,top=offset,bottom=0,left=0,right=0,borderType= cv2.BORDER_REPLICATE,value=0)
            frame_R = frame_R[:-offset,:]
        elif stereo_axis == 'y':
            frame_R = cv2.copyMakeBorder(frame_R,top=0,bottom=0,left=offset,right=0,borderType= cv2.BORDER_REPLICATE,value=0)
            frame_R = frame_R[:,:-offset]
        ...
        if not video_type == '2D': 
            ...
            frame_R_masked_for_stripes = frame_R_masked_padded[cyL+border_size-int(stripesL.shape[0]/2):cyL+border_size+int(stripesL.shape[0]/2),cxL-border_size:cxL+crop_size + border_size]
        ...
        # --------------------------    
            ...
            if stereo_axis=='y':
                ...
                stereo_image_R_cropped = stereo_image_R[int(border_size/2)+abs(cyL-cyR):int(border_size/2+crop_size)+abs(cyL-cyR), int(2*border_size - crop_size/2):int(2*border_size + crop_size/2)]
            ...
            cv2.imshow('testL',255-stereo_image_L_cropped) 
            cv2.imshow('testR',stereo_image_R_cropped)                
                
            # combine the two stereo images into an average, on the pixels on which both cameras agree there was a mouse
            if depth_being_opposite_not_sure_why:
                stereo_image_combined = ((stereo_image_R_cropped + (255 - stereo_image_L_cropped))*(frame_R_masked_cropped>0)*(frame_L_masked_cropped>0)).astype(np.uint8)
            else:
            ...
            cv2.imshow('test3',stereo_image_combined) 
        ...
        contoursL, big_cnt_indL, cxL2, cyL2, cntL = get_biggest_contour(frame_L_eroded)
        cv2.line(frame_L_masked_straight, (0,75), (150, 75),color = (255,255,255))
        cv2.imshow('erod',frame_L_eroded*2)
