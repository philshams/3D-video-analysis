...
iters = [1,3] #erosion and dilation iterations for mouse mask
...
if time_filter_width > 0:
    stereo_image_cropped_combined_gauss_prev4 = np.zeros((square_size)).astype(np.uint8)
    stereo_image_cropped_combined_gauss_prev3 = np.zeros((square_size)).astype(np.uint8)
    stereo_image_cropped_combined_gauss_prev2 = np.zeros((square_size)).astype(np.uint8)
    stereo_image_cropped_combined_gauss_prev = np.zeros((square_size)).astype(np.uint8)
    stereo_image_cropped_combined_gauss_cur = np.zeros((square_size)).astype(np.uint8)
...
while True:
    ...
    if ret:       
        ...
        reduced_contour_mask_L = contour_mask_L_padded[cyL:cyL+crop_size + int(border_size/2),cxL-border_size:cxL+crop_size + border_size]
        reduced_contour_mask_R = contour_mask_R_padded[cyL:cyL+crop_size + int(border_size/2),cxL-border_size:cxL+crop_size + border_size]
        ...
        frame_norm_L_masked_striped = frame_norm_L_masked_padded[cyL:cyL+int(2*border_size),cxL-border_size:cxL+crop_size + border_size] + stripe_maskL
        frame_norm_R_masked_striped = frame_norm_R_masked_padded[cyL:cyL+int(2*border_size),cxL-border_size:cxL+crop_size + border_size] + stripe_maskR
        ...
        frame_norm_R_cropped = frame_norm_R_masked[cyR-int(crop_size/2):cyR+int(crop_size/2),cxR-int(crop_size/2):cxR+int(crop_size/2)]
        frame_norm_L_cropped = frame_norm_L_masked[cyL-int(crop_size/2):cyL+int(crop_size/2),cxL-int(crop_size/2):cxL+int(crop_size/2)]    

        stereo_image_L_cropped = stereo_image_L[border_size - int(crop_size/2):border_size + int(crop_size/2),int(2*border_size) - int(crop_size/2):int(2*border_size) + int(crop_size/2)] #i might be wrong
        stereo_image_R_cropped = stereo_image_R[border_size - int(crop_size/2):border_size + int(crop_size/2),int(2*border_size) - int(crop_size/2)-(cxL-cxR):int(2*border_size) + int(crop_size/2)-(cxL-cxR)]
        ...
        if fra < 50:
            stereo_image_straight_save = cv2.resize(stereo_image_straight,(20,20))
            cv2.imshow('testpca',stereo_image_straight_save)
            depth_array[:,:,fra] = stereo_image_straight_save
        fra += 1
...
if save_data:
    ...
    if time_filter_width == 1:
        data_times = data_times - 1
    elif time_filter_width == 2:
        data_times = data_times - 2
