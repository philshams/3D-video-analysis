...
def prepare_data_for_model(pca_coeffs, num_PCs_used, position_orientation_velocity, disruptions, \
                           ...
                           standardize_pc_magnitudes, file_location_data_library, model_type, model_type_and_name_tag, session, session_num):
    ...
    # --------------------------------------------------------------
    # Normalize to 95th percentile of features in the first session
    # --------------------------------------------------------------
        if standardize_pc_magnitudes:
            for s in range(max(session_num)):
            
                if os.path.isfile(file_location_data_library + '_' + model_type + '_feature_standardization_' + model_type_and_name_tag + '.npy'):
                    feature_standard = np.load(file_location_data_library + '_' + model_type + '_feature_standardization_' + model_type_and_name_tag + '.npy')
                else:
                    feature_standard = np.percentile(data_for_model[session_num==s,:],95,axis=0)
                    print('saving feature standards from session ' + str(s))
                    np.save(file_location_data_library + '_' + model_type + '_feature_standardization_' + model_type_and_name_tag + '.npy', feature_standard)
            
                feature_magnitude_cur = np.percentile(data_for_model[session_num==s,:],95,axis=0)
                data_for_model[session_num==s,:] = data_for_model[session_num==s,:] * feature_standard / feature_magnitude_cur
...
def calculate_and_save_model_output(data, model, num_clusters, file_loc, model_type, suffix, do_not_overwrite):
    ...
    print(np.sum(components_binary,axis=0))
    print(np.percentile(data,95,axis=0))
    print(np.percentile(data,5,axis=0))
    print('')
...
def create_legend(num_PCs_shown, add_velocity, speed_only, add_change, add_turn):
    ...
    if add_velocity:
        ...
        if not(speed_only):
            legend_entries.append('angular speed')
    if add_change:
        legend_entries.append('change in pose')
    if add_turn:
        legend_entries.append('anglular speed')
