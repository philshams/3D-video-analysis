...
from learning_funcs import filter_features, create_legend, double_edged_sigmoid; import cv2; import pandas as pd
...
head_direction_full = np.ones(len(frames_all))*np.nan; head_direction_rel_to_shelter_show_full = np.ones(len(frames_all))*np.nan
...
distance_from_shelter_full[frames] = distance_from_shelter; head_direction_full[frames] = head_direction; head_direction_rel_to_shelter_show_full[frames] = head_direction_rel_to_shelter_show
...
while True:
    ...
    if ret:
        ...
        if out_of_bounds[frame_num]==0:
            cv2.putText(frame,'HD: ' + str(int(head_direction_rel_to_shelter_show_full[frame_num])),(200,100),0,1,255)
            ...
            cv2.circle(frame, (int(shelter_center[0]), int(shelter_center[1])), 10, (255,0,0), -1)
...
#        video.write(frame)
        ...
        if keystroke == ord('q'):
            ...
            mouse_vid.set(cv2.CAP_PROP_POS_FRAMES,frame_num - 10*(frame_rate>0) - (2*frame_rate<0))
            additional_pause = 1000*(frame_rate>0)
...
if save_flip_ind and loaded_corrected_array:
    position_orientation_velocity[flip_ind,4] = position_orientation_velocity[flip_ind,4] - 180* (2*(position_orientation_velocity[flip_ind,4] > 0) - 1)
    position_orientation_velocity[flip_ind,2] = -1*position_orientation_velocity[flip_ind,2]
    position_orientation_velocity[flip_ind,3] = -1*position_orientation_velocity[flip_ind,3]
    
    np.save(file_location_data_cur + '_position_orientation_velocity_corrected.npy', position_orientation_velocity)
