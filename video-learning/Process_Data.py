...
data_library_name_tag = 'streamlined_test'
...
straighten_upside_down_frames = False # use existing LDA and wavelet transform to straighten upside-down frames
...
if straighten_upside_down_frames:
    ...
    upside_down = False # flip video and save as a separate version, for later analysis
...
for session in enumerate(session_name_tags):
    ...
    if num_frames < 1:
        raise Exception(file_location_saved_data + '_data.avi' + ' not found')
    ...
    while flip_counter < len(flip_ind) or (not straighten_upside_down_frames):
        ...
        if ret:
            ...
            if show_images:
                ...
                if upside_down or straighten_upside_down_frames or True:
