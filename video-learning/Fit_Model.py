...
use_angular_speed_as_pseudo_PC = False

standardize_pc_magnitudes = True
...
filter_length = 6  # width of gaussian smoothing
sigma = 4  # standard deviation in frames of gaussian filter
...
if generate_new_model:
    ...
    disruptions = np.array([]); out_of_bounds = []; session_num = np.array([])
    ...
    for session in enumerate(session_name_tags):
        ...
        session_num = np.concatenate((session_num, np.ones(sum(out_of_bounds_cur==0))*session[0])).astype(int)
    ...
    np.save(file_location_data_library + '_' + model_type + '_settings_' + model_type_and_name_tag + '.npy',  # save settings
            [use_speed_as_pseudo_PC, True, False, use_angular_speed_as_pseudo_PC, num_PCs_used, window_size, 
             windows_to_look_at, np.max(data), standardize_pc_magnitudes])
    ...
    use_speed_as_pseudo_PC, _, _, use_angular_speed_as_pseudo_PC, num_PCs_used, window_size, windows_to_look_at, _, standardize_pc_magnitudes = \
...
for session in enumerate(session_name_tags):
    ...
    disruptions_cur[1:] = np.not_equal(out_of_bounds_cur[1:], out_of_bounds_cur[:-1])
    disruptions_cur = disruptions_cur[out_of_bounds_cur == 0]
    session_num = np.zeros(sum(out_of_bounds_cur==0)).astype(int)
